...
| `SUPPORT_EMAIL` | optional | Sets `Reply-To`. `EMAIL_FROM` cannot receive, so without this a user locked out by a spam filter who replies to their sign-in mail reaches nobody — the exact failure #47 names. Any monitored inbox; no receiving setup needed on the sending domain |
