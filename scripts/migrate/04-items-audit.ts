/**
 * Audit the shape of every lesson content item, across both content stores.
 *
 ...
 * illustrative.
 *
 * Two stores are surveyed, not one. Mongo is the original source, but the P2
 * migration moved content into Airtable and authoring continued there — the
 * Airtable `Lessons` table holds legacy lessons that no longer exist in Mongo.
 * Auditing Mongo alone would under-report the vocabulary, so both are profiled
 * and the per-type field sets are reported separately and unioned.
 */
...
async function airtableAll(table: string): Promise<AirtableRecord[]> {
  ...
  do {
    ...
    const res = await fetch(
      `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(table)}?${search}`,
...
async function main() {
  ...
  const airNew = new Profiler();
  const airLegacy = new Profiler();
  ...
  try {
    ...
    /* Airtable */
    const airLessonRecords = await airtableAll("Lessons");
    const airNewRecords = await airtableAll("NewLessons");

    for (const rec of airLessonRecords) {
      const slug = String(rec.fields.Slug ?? rec.id);
      for (const ex of parseArray(rec.fields.Exercises)) airLegacy.add(ex, slug);
      const cards = parseArray(rec.fields.Flashcards);
      const audio = parseArray(rec.fields.FlashcardsAudio);
      flashcardNotes.push(
        `| airtable | \`${slug}\` | ${cards.length} | ${audio.length} | ${cards.length === audio.length ? "aligned" : "**MISALIGNED**"} | ${cell(cards[0])} / ${cell(audio[0])} |`
      );
    }
    for (const rec of airNewRecords) {
      const slug = String(rec.fields.Slug ?? rec.id);
      for (const item of parseArray(rec.fields.Items)) airNew.add(item, slug);
    }

    /* ── output ─────────────────────────────────────────────────────── */

    console.log("# Mongo / Airtable `items[]` shape audit (CMS handoff Task 0)\n");
    console.log(
    ...
    console.log(`| airtable | \`Lessons\` | ${airLessonRecords.length} |`);
    console.log(`| airtable | \`NewLessons\` | ${airNewRecords.length} |`);

    console.log(
      "\n> **Mongo is no longer the complete source of truth.** The Airtable `Lessons` " +
        "table holds 9 legacy lessons; Mongo `lessons` holds 2. `MIGRATION_PLAN.md` " +
        "records 9 legacy lessons in Mongo at P2 migration time, so 7 were removed from " +
        "Mongo (or authored in Airtable) afterwards. The Payload import must source " +
        "legacy lessons from **Airtable**, not Mongo, or it will silently drop 7 lessons.\n"
    );

    /* Per-slug divergence between the two stores. */
    console.log("## Store divergence (same slug, both stores)\n");
    console.log("| family | slug | mongo | airtable | item JSON identical? |");
    console.log("| --- | --- | --- | --- | --- |");
    const divergenceRow = (
      family: string,
      slug: string,
      ...
      const same = a && b ? JSON.stringify(a) === JSON.stringify(b) : false;
      console.log(
        `| ${family} | \`${slug}\` | ${a ? `${a.length} items` : "**absent**"} | ` +
          `${b ? `${b.length} items` : "**absent**"} | ${a && b ? (same ? "identical" : "**DIFFERS**") : "n/a"} |`
      );
    };

    const mongoLessonBySlug = new Map(lessons.map((d) => [String(d.slug), d]));
    const airLessonBySlug = new Map(airLessonRecords.map((r) => [String(r.fields.Slug), r]));
    for (const slug of [...new Set([...mongoLessonBySlug.keys(), ...airLessonBySlug.keys()])].sort()) {
      const m = mongoLessonBySlug.get(slug);
      const a = airLessonBySlug.get(slug);
      divergenceRow(
        "legacy",
        slug,
        m ? (Array.isArray(m.exercises) ? m.exercises : []) : null,
        a ? parseArray(a.fields.Exercises) : null
      );
    }

    const mongoNewBySlug = new Map(newLessons.map((d) => [String(d.slug), d]));
    const airNewBySlug = new Map(airNewRecords.map((r) => [String(r.fields.Slug), r]));
    for (const slug of [...new Set([...mongoNewBySlug.keys(), ...airNewBySlug.keys()])].sort()) {
      const m = mongoNewBySlug.get(slug);
      const a = airNewBySlug.get(slug);
      divergenceRow(
        "newlessons",
        slug,
        m ? (Array.isArray(m.items) ? m.items : []) : null,
        a ? parseArray(a.fields.Items) : null
      );
    }
    ...
    renderProfiler("Airtable `NewLessons` → `Items`", airNew, "lessons");
    renderProfiler("Mongo `lessons` → `exercises[]`", mongoLegacy, "lessons");
    renderProfiler("Airtable `Lessons` → `Exercises`", airLegacy, "lessons");
    ...
    const legacyFields = new Set([...mongoLegacy.allFields(), ...airLegacy.allFields()]);
    ...
    const newFields = [...new Set([...mongoNew.allFields(), ...airNew.allFields()])].sort();
