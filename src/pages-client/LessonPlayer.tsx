...
// Inlined from the old client/src/services/api.ts, which no longer exists in
// the Next app. Fire-and-forget wrapper: runs an async handler and discards
// its promise so it can be used directly as a (void-returning) event handler.
const safe =
...
const Lesson: React.FC<{ lessonId: string; lesson: LessonDoc }> = ({ lessonId, lesson: lessonProp }) => {
  const router = useRouter();

  const [lesson, setLesson] = useState<LessonDoc | null>(lessonProp);
  ...
  useEffect(() => {
    ...
    setDebugInfo({
      ...
      receivedKeys: lessonProp ? Object.keys(lessonProp as any) : [],
      slug: (lessonProp as any)?.slug,
      _id: (lessonProp as any)?._id,
      flashcardsLen: ((lessonProp as any)?.flashcards || []).length,
      flashcardsAudioLen: ((lessonProp as any)?.flashcardsAudio || []).length,
      exercisesLen: ((lessonProp as any)?.exercises || []).length,
      exerciseTypes: ((lessonProp as any)?.exercises || []).map((x: any) => x?.type),
      exerciseImages: ((lessonProp as any)?.exercises || []).map((x: any) => ({
  ...
  useEffect(() => {
    if (!lesson || !lessonKey || !steps.length || resumedRef.current) return;
  ...
  function recordAttempt({
    ...
    if (lesson && createAttempt && lessonKey) {
