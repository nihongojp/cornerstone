...
const NewLessonPage: React.FC<{ slug: string; lesson: NewLessonDoc }> = ({ slug, lesson }) => {
  ...
  // each visit).
  //
  // This deliberately runs in an effect rather than during render. The
  // expansion shuffles, so computing it while rendering would produce one
  // order on the server and a different one during hydration — React would
  // see mismatched HTML. Running it after mount means the shuffle happens
  // exactly once, in the browser. `null` means "not expanded yet" and shows
  // the same loading state the fetch used to; an empty array after expansion
  // is a genuinely empty lesson.
  ...
  useEffect(() => {
    if (!slug || !items?.length || resumedRef.current) return;
  ...
  const handleNext = () => {
    if (isLast) {
      ...
      router.push(lesson?.nextSlug ? `/newlesson/${lesson.nextSlug}` : "/new-lessons");
      return;
    }
    setStep((s) => s + 1);
  };
