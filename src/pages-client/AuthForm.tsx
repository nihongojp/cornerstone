...
const AuthForm = (): React.ReactElement => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const redirectTo = searchParams.get("from") || "/new-lessons";

      // The session lives in an httpOnly cookie, so a full reload is no longer
      // needed to sync the header and route guards — refresh() re-renders the
      // server components with the new session.
            ...
            <form onSubmit={handleSubmit}>
              ...
              {mode === "login" && (
                <Box mt={2} textAlign="center">
                  <Button variant="text" size="small" onClick={() => router.push("/forgot-password")}>
                    Forgot password?
                  </Button>
                </Box>
              )}
