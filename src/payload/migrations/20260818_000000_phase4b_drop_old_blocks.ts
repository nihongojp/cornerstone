 ...
 *
 * It was written to land with the rest of Phase 4b and could not, because
 * Payload runs every pending migration in one batch: there is nowhere inside a
 * single deploy to put the `content:import` this file's guard is waiting for.
 * Production's content has never been re-imported — `migrate-production.yml`
 * applies schema only — so on the merge of the Phase 4b/5 pull request the
 * guard below correctly refused, and said so in CI against a Neon branch forked
 * from `production`: 143 exercises across all five lessons.
 *
 * So it waits for a release of its own. The order across the two, and the
 * reason the timestamp moved forward past Phase 5's migration:
 *
 *   release 1   20260817_214000_phase4b_spotlight_layout   (expand)
 *               20260817_223731_phase5_autosave_roles
 *               npm run content:import -- --yes            (against production)
 *   release 2   this file                                  (contract)
 *
 * Between the two, production carries the old block tables holding rows that
 * nothing can read: the re-import in release 1 writes the new library rows and
 * strands the old ones, because Payload deletes a block array's child rows only
 * for types in the *current* config and this branch removed those definitions.
 * That is not a leak to tidy up in a hurry — for one release it is the only
 * remaining copy of the pre-4a content, which is what makes release 1
 * recoverable. This file is what finally collects them.
 *
 * There is deliberately no `.json` snapshot beside this migration. The config
 * stopped defining the old blocks in release 1, and `20260817_223731`'s
 * snapshot already records that; this file changes the database to match and
 * changes nothing about the config, exactly like the expand half above.
 *
 * Full sequence, including what to run against production and in what order:
 * `docs/production-merge-runbook.md`.
 *
