...
import { normalizePhonemeToken } from "./phoneticFeatures";
...
// In the container image this cache is populated at docker build time (see
// Dockerfile) so a cold start never has to hit the Hub for weights.
...
/** True once the model, processor and vocab are loaded and inference can run without waiting. */
export function isPhonemeRecognizerReady(): boolean {
  return ready;
}
